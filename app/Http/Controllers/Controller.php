...
class Controller extends BaseController
{
    ...
    public function storeFiche(Request $request) {
        ...
        if ($horaire != null) {
            ...
            if ($request->input('motif_test') == 'voyage') {
                ...
                //Soustraire deux jours et verifier la disponibilité à nouveau
                $date_dispo_init->addDays(2);
                $date_dispo_init->toDateString();

                $date_dispo =  Horaire::where('date', '>=', $date)->orderBy('id', 'DESC')->first();
    ...
    public function update(Request $request, $id){
        ...
        $fiche_patient = Fiche::find($id)->first();
        $horaire = Horaire::find($fiche_patient->id);
        $date_initiale = $horaire->date.' '.$horaire->horaire;

        $tel = $horaire->gsm;
        $ddn = $fiche_patient->ddn;
        $message = 'Votre rendez-vous est fixé à la date : '. $date_initiale;

        return view('rendez_vous_edit', [
            'message' => $message,
            'tel' =>$tel,
            'ddn' => $ddn,
            'fiche' =>$fiche_patient
    ...
    public function modifierFiche(Request $request, $id) {
        ...
        if ($request->input('motif_test') == 'voyage') {
            ...
            //voir disponibilité initiale de la date
            $date_dispo_init = Horaire::where('date', '<=', $date)->orderBy('id', 'DESC')->first();

            if($date_dispo_init == null ) {

                $message = 'Votre rendez-vous demeure à sa date initiale : '. $date_initiale;
